 ...
 */
  ...
  int j; /*Variable to ensure it has a 20% chance to run*/
  int p;
  u64_t time;
  u32_t rand;
  ...
  for (q=0; q < NR_SCHED_QUEUES; q++) {	
	...
	j = rand % 20;
	p=q;
	if((q> 7) && (j<4) ){ /* Makes it a 20% chance that this will have a higher priority*/ 
		p=((rand*q) %NR_SCHED_QUEUES); 
/*Randomizes the priority of Q, but keeps it within the range of 1-16 because of modding with the NR_SCHED_QUEUES which is 16 in this scenario. Tasks 0-7 are kernal tasks, and 8-16 are user processes and tasks*/	
	}
